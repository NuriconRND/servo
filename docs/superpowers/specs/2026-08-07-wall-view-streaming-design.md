...
> 구현체와 최신 문서는 `F:\20260609_SDWall_BrowserTest\20260606_multigpu_browser\wall_view\`
> (독립 git 저장소, `tools/dx_wall_probe`와 같은 방식)에 있다. wall_view는 Servo 엔진과
> 아무 관계가 없어 이 저장소에 둘 이유가 없었다.
>
> **이 문서는 설계 시점 기록으로만 남긴다.** 갱신은 이관된 저장소의 `docs/`에서 이뤄지므로,
> 최신 내용이 필요하면 그쪽을 볼 것.
