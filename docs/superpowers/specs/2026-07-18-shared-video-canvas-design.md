...
**후속 티켓(백로그, 비차단 — 최종 리뷰 권고)**: 45타일 presents 53.3/s(30Hz 미수렴 — 도착 분산으로 더티스킵 배칭 미발동)의 해소 = 컴포짓 케이던스 양자화/frame-ready 도착 배칭(비디오 카데이던스 수렴) — §14 동기 중 미달성으로 남은 AMD GPU 감축 목표의 다음 증분. 판독은 그때까지 케이던스 보정 기준(§14.6 이탈 1).
