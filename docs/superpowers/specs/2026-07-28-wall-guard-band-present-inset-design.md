...
> **실행 후 정정(2026-07-28, 최종 리뷰 지적):** 근거 (4)는 **초기 구현이 제공하지 못했다.**
> `present_inset`의 유일한 독자가 DComp 컴포지터였고, blit 경로(`wall_tile_render_insets`)는
> `WallLayout::tile_render_insets`를 자기가 다시 호출했다. 즉 공유된 것은 **산식**이고 **값**이
> 아니었다 — 두 값은 계산 시점이 달라(생성 시 스냅샷 vs 매 프레임 라이브 hidpi) 혼합 DPI에서
> 갈릴 수 있었고, `ScaleFactorChanged` 재주입 경로도 없었다. 후속 fix wave에서 blit 경로를
> `present_inset` 리더로 전환하고 DPI 변경 시 재주입을 추가해 (4)가 비로소 성립하게 했다.
> **교훈: "정본을 만든다"는 설계는 기존 계산 지점을 리더로 바꾸는 작업까지 포함해야 하며,
> 계획에 그 단계를 명시하지 않으면 값이 둘로 남는다.**
...
**gcd(타일 치수)마다** 절대 가상좌표 숫자를 렌더한다(가로/세로 양쪽). 각 타일 창의 최상단·최좌단에 읽히는
...
> **실행 중 정정:** 초안은 라벨 간격을 128px로 잡았는데 `1080 % 128 = 56`이라 **세로 분할의 타일
> 경계 y=1080에 라벨이 없었다** — 위 판정 규칙("구석 라벨 = 타일 원점")이 계획 자신의 상수로는
> 성립하지 않았다는 뜻이다. 라벨 간격은 반드시 **모든 타일 원점을 나누는 값**이어야 한다.
> 이 월 구성에서는 `gcd(1920, 1080) = 120`. 미세 눈금 8px은 1920·1080을 둘 다 나누므로 유지
> (가드밴드 32px = 정확히 4칸). 주눈금도 120px로 맞춰 타일 경계가 항상 주눈금선 위에 오게 했다.
> **일반화: 계측기의 눈금 간격은 측정 대상의 경계 좌표를 반드시 포함해야 한다.**
