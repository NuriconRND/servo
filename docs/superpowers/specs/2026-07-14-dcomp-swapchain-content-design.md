...
end_frame:
  ① 레이어 컬링: add_surface는 아래→위 순서이므로 목록을 역순(최상위부터)으로
     ...
     visual을 트리에서 제외 (컬링의 **주체**는 불투명 서피스만 — 위의 불투명이
     완전히 덮는 서피스는 알파여도 비가시이므로 제외 대상이 될 수 있음; 진단용
     컬링-오프 env 제공)
...
- 별도 알파 슬라이스(fixed 오버레이 등): 가상 서피스로 남아 DWM이 겹침 —
  내용 불변 시 draw 0. 위에 완전 피복 불투명이 없는 한 컬링되지 않음(피복되면
  비가시라 제외해도 시각 등가).
...
### 컬링 규칙 — 스펙 문면 정정 (최종 리뷰)

컬링 규칙 — 승인 스펙 문면("알파 절대 비컬링")과 달리 구현·계획은 "주체=불투명"
조건(완전 피복된 알파는 비가시라 제외 = 시각 등가, 최종 리뷰 검증). §5.2/§5.3
문구를 구현에 맞게 정정함.
타일 그리드 조밀성 가드(Fix 1) 추가 — 승격/regen 판정에 타일 수×타일 면적==extent
면적 조밀성 검사를 넣어, 비조밀 타일 그리드에서 스왑체인 구멍이 미정의 픽셀로
Present되는 이론적 위험을 차단했다(현 워크로드에서는 미발생).
