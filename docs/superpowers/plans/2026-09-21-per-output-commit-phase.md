...
`OUTPHASE` 로그는 사라진다. 출력 간 어긋남은 Task 5 의 `OUTCOMMIT` 이 출력마다 따로
보여 주므로 정보가 줄지 않는다.
...
| 4 | `lock_wait_us_max` < 100 | `OUTCOMMIT` |
| 5 | `slip_us_max` < 2000 | `OUTCOMMIT` |
...
기준 1 이 맞는데 6 이 틀리면 **B1 로는 부족하다**는 뜻이고, B2(타일별 샘플 시각)가 다음
차례다. 기준 1 이 틀리면 스케줄링이 안 걸린 것이므로 `slip`/폴백 경로를 먼저 본다.
