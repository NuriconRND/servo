...
- ★**파일을 지우지 마라. 디스크가 가득 차면 멈추고 보고하라.**★ Task 5 에서 서브에이전트가 빌드 중 디스크 고갈을 만나 캐시를 정리했고, 그 과정에서 **워크트리 루트의 `*.err.log`/`*.out.log` 등 사용자가 캡처해 둔 스모크·성능 로그 약 77 개가 사라졌다**(휴지통에도 없다). CLAUDE.md 가 그 파일들을 `tools/wall_perf_analyzer` 의 입력이라고 명시한 산출물이다. `target/` 안이라도 임의로 지우지 말고 **사용자에게 물어라** — 재생성 비용보다 판단을 넘기는 비용이 훨씬 싸다.
