...
- ★**env 를 pref 로 옮길 때 `etc/multigpu/*.ps1` 운용 스크립트를 반드시 함께 고친다.**★ `run_video_wall_d3d11.ps1`, `run_video_wall_d3d11_debug.ps1`, `package_run_wall.ps1` 이 `$env:SERVO_* = ...` 로 노브를 켜고 servoshell 을 띄운다. 셸이 pref 를 **무조건 주입**하게 되는 순간 그 env 는 죽고 **스위치가 조용히 무효**가 된다 — 엔진은 경고도 안 찍고, 스크립트가 뒤늦게 찍는 경고는 "폴백이 일어났다" 고 잘못 말해 원인 추적을 반대 방향으로 유도한다. Task 3 리뷰에서 실제로 이 형태로 `-DComp` 와 `SERVO_WIN_VSYNC` 가 죽은 것이 발견됐다. `--pref <이름>=<값>` 인자 전달로 바꿔라.
- ★**파일을 지우지 마라. 디스크가 가득 차면 멈추고 사용자에게 보고하라.**★ Task 5 에서 서브에이전트가 빌드 중 디스크 고갈을 만나 임의로 캐시를 정리했고, 그 과정에서 **워크트리 루트의 `*.err.log`/`*.out.log` 등 사용자가 캡처해 둔 스모크·성능 로그 약 77 개가 사라졌다**(휴지통에도 없다). CLAUDE.md 가 그 파일들을 `tools/wall_perf_analyzer` 의 입력이라고 명시한 산출물이다.
  - ★**더 나쁜 것은 순서였다**★ — `target/debug/incremental`(71.5 GB, 크레이트 해시 디렉터리 427 개)이 **손도 안 댄 채 남아 있는데** `target/debug/deps`(실제 컴파일 산출물, cargo 가 링크에 쓴다)를 4203 개 스윕하고 루트 로그까지 건드렸다. `incremental` 은 **순수 컴파일러 캐시**라 아무것도 링크하지 않고, 지워도 다음 빌드가 비증분으로 한 번 도는 것이 전부다. 이 프로젝트에는 형제 워크트리에서 같은 정리로 57.87 GB 를 회수한 **선례까지 있었다.**
  - **디스크가 부족하면**: (1) 멈추고 **사용자에게 보고**한다. (2) 승인을 받으면 **`target/debug/incremental` 부터** 지운다 — 대개 그것으로 충분하다(2026-08-12 실측 52 GB 회수). (3) `deps` 스윕·`cargo clean`·루트 파일은 **그다음이고 각각 따로 승인**을 받아라. ★**`target/` 밖은 어떤 경우에도 지우지 마라.**★
