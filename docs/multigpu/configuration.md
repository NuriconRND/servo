...
2026-09-08~10 의 애니메이션·누수·크래시 작업에서 **8 개가 더 늘었다**(각 표에서 `(2026-09)`
표시). 이것들도 옛 환경변수가 없는 새 pref 다. 그 작업이 남긴 차후 과제는
[`review_followups_2026-09-10.md`](review_followups_2026-09-10.md) 에 있다 — 이 표의
`F1`/`F2`/`F5` 참조가 그것이다.
...
| `gfx_paint_side_animations_enabled` (2026-09) | bool | **`true`** | 킬스위치. CSS `opacity`/`transform` 애니메이션을 **페인트 측이 직접** 재생할지. 켜면 레이아웃이 그 값을 앞으로 `gfx_paint_side_animation_horizon_ms` 만큼 샘플링해 세그먼트로 만들고 WebRender `PropertyBinding` 에 묶는다 — 스크립트가 멈춰 있어도 애니메이션이 흐른다(그것이 이 기능의 목적이다). ★값을 얹는 것이지 프레임을 만드는 것이 아니다★ — 프레임은 그 주기에 **남이** 안 냈을 때만 낸다(`painter.rs` `perform_updates`). `=false` 면 예전처럼 스크립트가 매 프레임 디스플레이 리스트를 내야 움직인다. |
| `gfx_paint_side_animation_horizon_ms` (2026-09) | i64 | `3000` | 위 샘플링을 앞으로 얼마나 내다볼지. **스크립트가 얼마나 오래 멈출 수 있는가에 대한 내기**이고, 그 뒤로는 페인트가 마지막 값을 유지한다. 샘플 간격은 1/60 초 고정, 한 애니메이션당 상한 256 샘플(`paint_animation.rs` `MAX_SAMPLES`)이라 기본값은 약 181 샘플이다. ★비용은 프래그먼트 수에 비례한다★ — 애니메이션 프래그먼트 하나마다 디스플레이 리스트 빌드당 그만큼 스타일 질의가 돈다. |
| `gfx_paint_side_animation_tick_divisor` (2026-09) | i64 | `4` | 페인트가 **이미 재생 중인** 애니메이션에 대해 스크립트 틱을 몇 프레임에 한 번 요구할지. 화면의 값은 틱 사이에 페인트측 예측이 만들므로, 틱은 그 예측을 다시 맞추고 애니메이션 이벤트를 쏘는 데만 필요하다. ★틱 하나가 문서 전체 리플로 하나다★ — 실측(4-GPU 월, 2026-09-08): 애니메이션이 도는 동안 초당 122 리플로 대 없을 때 54, 전환 순간 리플로 한 번이 14.5ms. `1` 이면 매 프레임(예전 동작). 페인트가 재생하지 **않는** 애니메이션의 틱은 이 값과 무관하게 그대로 나간다 — 늦추면 그건 진짜로 느려진다. |
| `gfx_painter_detached_queue_limit` (2026-09) | i64 | `8` | 팬아웃한 이미지 갱신을 페인터 스레드에 **논블로킹**으로 보낼 때 처리되지 않은 채 쌓일 수 있는 최대 개수. 넘으면 그 요청을 **버린다**(`IMGUPDRATE ... dropped=`). 메인 스레드가 페인터를 기다리며 굳던 것을 없애면서 역압까지 같이 없애 버리지 않으려고 둔 상한이다. `0` 이면 무제한(버리지 않음). ★버려지는 것이 비디오 프레임 갱신뿐이라는 보장이 없다★ — 같은 배치에 `AddImage`/`DeleteImage` 가 섞일 수 있고 그것들은 멱등이 아니다(차후 과제 F1, `review_followups_2026-09-10.md`). |
...
| `media_release_detached_player` (2026-09) | bool | **`true`** | 문서에서 빠진 `<video>`/`<audio>` 의 미디어 파이프라인을 그 자리에서 놓을지. ★사양은 제거 시 **일시정지만** 하라고 한다★ — 그런데 벽 앱은 구성 전환마다 요소를 지우고 새로 만들고, 지워진 요소가 GStreamer 파이프라인과 D3D11 링을 붙든 채 남으면 그것이 영영 쌓인다(실측: 링 그룹 24 → 78 → 156, 커밋 메모리 844MB → 24GB). 파이프라인이 콜백으로 요소를 잡으므로 요소도 수집되지 않는다. 켜면 재생 위치를 유지한 채 놓고, 다시 문서에 들어오거나 `play()` 하면 그 지점에서 다시 연다. ★이 pref 가 캡처 해체 경로를 **처음 도달 가능하게** 만들었다★ — 그전에는 파괴가 GC 로 미뤄져 있었다(2026-09-10 크래시, 해결됨). |
| `media_ring_stage_first_frame` (2026-09) | bool | `false` | D3D11 링의 **첫 소비**에서 프레임 하나를 매핑된 메모리로 복사해 둘지. 켜면 첫 프레임이 곧바로 보이지만 그 복사가 렌더 스레드에서 돈다 — 실측(FHD I420 3.1MB): 스테이징 있는 최초 소비 12.6~23.9ms, 없는 것 6.5~10.9ms. 16.6ms 예산에서 그 한 번이 프레임을 통째로 넘긴다. 꺼 두면 프로듀서가 채울 때까지(대개 한두 프레임) 그 영상만 비워 두고 벽은 60fps 를 유지한다. |
| `media_ring_init_max_per_frame` (2026-09) | i64 | `1` | 한 **렌더 프레임** 안에서 새 비디오의 링을 몇 개까지 초기화할지. 여러 영상이 동시에 시작할 때 그 비용이 한 프레임에 몰리는 것을 막는다(`MEDIALOCKDEFER`). `0` 이하면 무제한. ★2026-09-10 기준 실행 로그에서 이 상한이 걸린 적이 한 번도 없다(`MEDIALOCKDEFER` 0 건)★ — 다른 수정으로 압력이 사라진 것인지 발동 조건이 틀린 것인지 확인이 필요하다(차후 과제 F5). |
| `dom_webgl_idle_context_reclaim_ms` (2026-09) | i64 | `3000` | 어느 타일에도 이만큼 합성되지 않은 WebGL 컨텍스트의 **GPU 자원을 놓는다**. `0` 이하면 회수하지 않는다(예전 동작). ★벽에서는 논리 컨텍스트 하나가 타일마다 백엔드 컨텍스트와 스왑체인을 따로 갖는다★ — 보이지 않는 캔버스 하나가 GPU 자원을 네 벌 잡는다. 놓을 때 표준대로 `webglcontextlost` 를, 되살릴 때 `webglcontextrestored` 를 페이지에 보낸다. ★판정 기준이 "합성됐는가"라 일반 페이지에는 과감하다★ — 스크롤 밖이거나 애니메이션이 멈춰 재합성되지 않는 캔버스도 대상이고, 두 이벤트를 구현하지 않은 페이지는 죽은 캔버스가 된다(차후 과제 F2: 기본 off + 벽에서만 켜기 검토). |
