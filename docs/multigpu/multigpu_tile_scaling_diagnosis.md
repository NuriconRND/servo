...
## ★★2026-08-27 — 격차의 정체: Servo 가 아니라 "한 프로세스 안에 있다"는 것★★

54 x FHD30, 같은 토폴로지·같은 페이싱·같은 장비. ***프로세스 경계 하나만 다르다:***

| 구성 | 총 CPU | 영상당 | 단일스레드 천장 대비 |
|---|---|---|---|
| 54 개 별도 프로세스 | 22.60 코어 | **0.418** | 1.19 배 |
| **1 개 프로세스, 54 체인** | 54.29 코어 | **1.005** | **2.83 배** |

***2.4 배가 순전히 프로세스 경계에서 온다.*** 그리고 Servo 월의 실측이 0.99~1.37 로
단일 프로세스 기준선과 사실상 같다 — ***Servo 가 얹는 몫은 거의 없다.***

이것으로 이 조사 내내 돌린 노브들이 왜 전부 잔돈이었는지가 설명된다: 파이프라인 토폴로지
(uridecodebin3), 페이싱(thread), 백버퍼 수, 디코더 스레드 수, 픽처 타일 크기, present 몰아치기 —
전부 이 2.4 배 안에서 놀았다.

### 후보 셋 (아직 미규명)

1. **힙 경합** — 54 개 디코더가 한 힙을 공유한다. 초당 54x30 = 1620 개 프레임 버퍼의 할당/해제.
2. **GStreamer 전역 상태** — caps/structure 인터닝, 레지스트리, GType 락. 프로세스당 싱글턴이다
   (***공유 `GstSystemClock` 과 같은 계열***).
3. **NUMA first-touch** — 단일 프로세스의 힙은 한 노드에 몰리는데 스레드는 두 소켓에 흩어진다.
   별도 프로세스면 각자 자기가 도는 노드에 메모리를 잡는다.

### 가르는 방법: `measure_decode_only.ps1 -Processes N`

54 체인을 N 개 프로세스에 라운드로빈으로 나눠 담는 옵션을 넣었다. ***프로세스당 체인 수에
따라 영상당 비용이 완만히 변하면 경합(1·2), 계단처럼 떨어지면 구조적(3)***이다.

    -Processes 1  (= -SingleProcess)   54 체인 1 프로세스
    -Processes 2 / 3 / 6 / 9 / 18      27 / 18 / 9 / 6 / 3 체인씩
    -Processes 54 (= 기본)             1 체인씩

곡선이 어디서 꺾이는지가 다음 작업을 정한다. 완만하면 할당자/전역 락을 파고, 계단이면
메모리·스레드 친화도를 판다.


## ★★2026-08-27 (2) — 프로세스 분할 곡선: 계단이다, 경사가 아니다★★

54 x FHD30 을 N 개 프로세스에 나눠 담은 결과(같은 토폴로지·페이싱·장비):

| 프로세스 | 체인/프로세스 | 총 CPU | 영상당 | 1 개 대비 |
|---|---|---|---|---|
| **1** | 54 | 54.06 | **1.001** | 100% |
| **2** | 27 | 25.79 | **0.478** | **47.8%** |
| 3 | 18 | 31.74 | 0.588 | 58.7% |
| 6 | 9 | 24.85 | 0.460 | 46.0% |
| 9 | 6 | 23.50 | 0.435 | 43.5% |
| 18 | 3 | 21.97 | 0.407 | 40.7% |
| 54 | 1 | 22.25 | 0.412 | 41.2% |

작업 관리자 육안 관찰("1->2 에서 절반 이상 감소, 이후 조금씩")과 정확히 일치한다.

### ★힙 경합·전역 락 가설 반증★

그것이 원인이라면 비용이 ***프로세스당 체인 수에 비례***해야 한다. 실제로는
***한 프로세스에 27 개를 넣어도 1 개만 넣은 것과 거의 같다***(0.478 vs 0.412).
54->27 에서 전부 회수되고 27->1 은 13% 밖에 더 주지 않는다. 변수는 체인 수가 아니라
***프로세스가 몇 개인가***다.

홀수가 짝수보다 나쁜 것도 맞아떨어진다 — 3 개(0.588) > 2 개(0.478), 9 개(0.435) > 6 개(0.460)…
***3 개는 두 소켓에 2/1 로 갈려 한쪽이 몰린다.***

### 남은 것: 프로세스 단위 배치/메모리 지역성

단일 프로세스의 힙은 first-touch 로 한 노드에 잡히는데 스레드 54 개는 두 소켓에 흩어진다 →
절반이 매 프레임 원격 메모리를 읽는다. 프로세스가 갈리면 각자 자기 노드에 메모리를 잡는다.

***월이 `-NumaNode auto` 로 이미 고정돼 있는데도 0.99 인 이유도 이것으로 설명된다*** —
`start /NODE` 는 하드 고정이 아니라 ***선호***다. 54 코어를 원하는 프로세스는 노드의 40 개에
못 들어가 스레드가 건너가고, ***메모리는 노드 1 에 남는다.*** 최악의 조합이다.

닭과 달걀이다: 비효율 -> 54 코어 필요 -> 넘침 -> 원격 접근 -> 계속 비효율.
***0.412 라면 같은 54 영상이 22 코어이고 노드 하나(40)에 여유 있게 들어간다.***

### 조치: `measure_decode_only.ps1 -NumaNode <n>` (하드 친화도)

`start /NODE n /AFFINITY <그 노드 전부>` — 마스크는 노드 안에서 해석되므로 all-ones 가 곧
그 노드 전체다. ***넘침을 금지하면 그 고리가 끊기고 좋은 상태로 수렴하는지***를 본다.

    -Processes 1 -NumaNode 1     54 체인, 노드 1 에 하드 고정
    -Processes 1                 (대조군, 현행)

1.0 에서 0.4 대로 떨어지면 원인은 지역성이고, 월의 근본 조치는 ***프로세스 하드 친화도***가
된다. 그대로 1.0 이면 지역성도 아니고 남는 것은 프로세스 단위 무언가(GStreamer 전역 상태
계열)이며, 그때는 ***멀티프로세스 아키텍처가 유일한 길***이라는 뜻이다.


## 2026-08-27 (3) — 한 그룹에 가두면 절반쯤 회수된다 (전부는 아니다)

54 x FHD30, 같은 토폴로지·페이싱, ***ProcessorAffinity 를 OS 에서 되읽어 확인한*** 실행:

| 구성 | 영상당 코어 | 천장 대비 |
|---|---|---|
| 1 프로세스, 고정 없음 | 0.723 ~ 1.005 | 2.05 ~ 2.84 배 |
| **1 프로세스, 한 그룹에 하드 고정** | **0.600** | **1.69 배** |
| 54 프로세스 | 0.418 | 1.19 배 |

***지역성은 원인의 일부다.*** 프로세스를 한 그룹(40 논리 CPU)에 가두면 0.72 -> 0.60 으로
내려간다. 하지만 ***54 프로세스의 0.418 에는 못 미친다*** — 두 소켓에 흩어져 도는 54 개
프로세스가 한 소켓에 갇힌 단일 프로세스보다 여전히 싸다. 즉 프로세스 경계가 주는 이득
중 지역성으로 설명되는 것은 절반 정도이고, 나머지는 다른 데 있다.

***주의: 고정 없는 값이 0.723 과 1.005 로 크게 흔들린다.*** 판정은 같은 세션에서 등을
맞대고 돌린 쌍으로만 해야 한다.

### 계측 규율: 고정 노브는 자기 효과를 증명해야 한다

`-Confine` 은 이제 설정 후 OS 에서 마스크를 되읽어 대조하고, 다르면
***측정 자체를 거부***한다. 성공하면 `confined : 0xffffffffff (40 processors), read back
from the OS` 를 찍는다.

이 규율이 없어서 세 라운드를 날렸다. `start /AFFINITY <mask>` 는 말없이 거부당했고,
증상은 "gst-launch did not start" 뿐이라 마스크 형식·명령줄 길이·GStreamer 버전을 차례로
헛짚었다. ***고정된 척하는 실행이 만들어내는 숫자는 데이터처럼 보인다.***

### `start /NODE` 의 마지막 함정

래퍼가 자기 stdout/stderr 를 남기게 하자 원인이 한 줄로 나왔다:
`errorlevel 9059`, `cannot find the file powershell`. `start /B` 는 자체 탐색을 쓰며
확장자 없는 `powershell` 을 해결하지 못한다. 전체 경로로 고쳤다.


## ★★2026-08-27 (4) — 배치가 전부다: 하드 고정만으로 디코드 40% 감소★★

54 x FHD30, 같은 토폴로지·페이싱, 한 세션에서 20 회. `-Confine` 은 마스크를 OS 에서
되읽어 확인한다.

| 구성 | 영상당 코어 (실행별) | 군집 |
|---|---|---|
| 1 프로세스, 고정 없음 | 1.001 1.004 1.005 1.007 (0.793 1 회) | **~1.005** |
| 1 프로세스, **하드 고정** | 0.597 0.601 0.604 (0.735 2 회) | **~0.60** |
| 2 프로세스, 고정 없음 | **1.009 1.010 | 0.510 0.517 0.534** | ***둘로 갈림*** |
| 2 프로세스, **하드 고정** | 0.520 0.533 0.537 0.539 (0.735 1 회) | **~0.53** |
| 54 프로세스 | 0.418 | 0.418 |

### ★2 프로세스가 동전 던지기다★

고정 없이 두 프로세스를 띄우면 ***같은 노드에 떨어져 단일 프로세스만큼 나쁘거나(1.01),
다른 노드로 갈려 절반(0.52)이거나*** 둘 중 하나다. 라운드 19~24 에서 월이 보인 양안정과
정확히 같은 현상이고, ***고정을 걸면 갈림이 사라진다.***

### 월에 그대로 적용된다

***월의 디코드는 0.99~1.37 로 최악 군집(1.005)에 정확히 앉아 있다.*** 그리고 단일
프로세스도 하드 고정만으로 0.60 까지 내려간다 — ***아키텍처 변경 없이 40% 감소***다.
54 영상 환산: 0.99 x 54 = 53 코어(불가능) -> ***0.60 x 54 = 32 코어***(노드 하나 40 개 안).

`-NumaNode auto` 는 `start /NODE` 뿐이라 ***선호이지 고정이 아니다.*** 그래서
`run_wall_dist.ps1 -Confine` 을 추가했다: 기동 직후 ProcessorAffinity 를 자기 그룹 전체로
설정하고 OS 에서 되읽어 `confined : 0x... , read back from the OS` 를 찍는다. 실패하면
"이 실행은 고정되지 않았다"고 명시한다(조용히 넘어가지 않는다).

### 아직 남는 것

54 프로세스의 0.418 에는 고정만으로 못 미친다(0.60). ***프로세스 경계 이득의 절반쯤은
지역성으로 설명되고 나머지는 설명되지 않는다.*** 다만 월 입장에서 지금 필요한 것은 그
나머지가 아니라 눈앞의 40% 다.

### 곁들여 고친 것

`make_wall_dist.ps1` 이 dist 폴더를 ***지우지 않고 비운다.*** dist 안에 있던 셸이 디렉터리
핸들을 잡고 있으면 내용물은 지워지는데 폴더에서 실패해, ***속이 빈 dist 가 패키징된 것처럼
남았다.*** "exe 가 없다"는 혼란스러운 증상으로 여러 번 시간을 먹었다.


## ★★2026-08-27 (5) — 45 영상 정상 달성, 54 영상의 벽은 "노드당 40 코어"★★

`-Confine` 을 월에 적용한 실측(singlegpu 5760x2160, uridecodebin3, thread 페이싱):

| | 영상 | escape | 영상 fps | **pts** | cadence | render p50 | 프로세스 | 디코드 | 머신 |
|---|---|---|---|---|---|---|---|---|---|
| **B** | **45** | off | **30.0** | **1.00** | 31.0 | 33.9 ms | 34.35 | 0.78 | **43.3%** |
| A | 54 | external | 30.0 | 1.00 | **2.2** | **356.9 ms** | 39.74 | 0.74 | 50.1% |

### B — 45 영상은 된다

***영상당 정확히 30.0fps, pts_rate 1.00, 머신 43.3%.*** 45 영상에서 pts 를 재본 것은 이번이
처음이고, 1.00 이 확인됐다. 고정 없는 45 영상(37.32 코어)보다 CPU 도 8% 낮다.

### A — 54 영상은 산술적으로 안 들어간다

    0.74 코어/영상 x 54 = 40.0 코어
    그룹 하나           = 40 논리 코어
    프로세스 실측        = 39.74   <- 디코드가 그룹을 정확히 다 먹었다

디코드 자체는 완벽하다(pts 1.00, 30fps). ***렌더러에게 남은 것이 0*** 이라 cadence 2.2/s,
render 357ms 로 무너졌다. `escape=external` 이 디코드를 0.80 -> 0.74 로 낮췄지만 그만큼
convert+present 를 ***하필 굶고 있는 그 렌더러 스레드로*** 옮겨 역효과였다.

***54 영상의 벽은 GPU 도 메모리 대역도 아니고 "한 노드에 40 코어뿐" 이다.***
대략 48~50 영상이 한 노드로 가능한 상한이다(0.75 x 50 = 37.5 + 렌더러 ~= 39).

### 이것이 스레드별 친화도가 옳은 방향임을 뒷받침한다

    고정 없음 : 디코드가 두 소켓을 떠돌며 힙은 한 노드   -> 0.99, 용량 80
    프로세스 고정 : 지역성은 얻고                        -> 0.74~0.80, ***용량 40***

***둘 다 얻으려면 스레드 단위로 나눠야 한다*** — 디코드는 80 코어 전부, GPU 를 만지는
부분(업로드·convert·present)만 GPU 노드에. 다만 지금은 ***디코드와 D3D11 업로드가
같은 스레드(ServoGstVideo)에 직렬로 묶여 있어*** 나눌 수가 없다. 그 분리가 선결 조건이다.

### 현재 위치

- ***45 영상: 달성.*** 30.0fps, pts 1.00, 머신 43.3%.
- 54 영상: 디코드는 되고 합성이 굶는다. 한 노드 용량이 벽.
- 대조군(비 Servo)은 같은 장비에서 54 영상을 50~70% 로 돈다. 우리는 45 영상 43% 다.


## ★★2026-08-27 (6) — 업로드도, 콜백 전체도 아니다: 우리 코드는 영상당 0.01 코어다★★

### 먼저 업로드를 쟀다 (D3D11PROF, 45 영상)
...
30fps 환산 ***0.047 코어/영상***. 스레드가 실제로 쓰는 0.76 의 ***6%*** 다.
***업로드를 통째로 다른 스레드로 옮겨도 0.76 -> 0.71 이라 아무 의미가 없다*** —
"디코드와 업로드가 한 스레드에 묶여 있다"는 진단은 맞았지만, 옮길 물건이 작았다.
***설계 전에 크기부터 잰 것이 이번엔 제대로 작동했다.***

### 그래서 콜백 전체를 쟀다 (SINKPROF 신설)

`SERVO_MEDIA_SINK_PROF=1` / 런처 `-SinkProf`. appsink 콜백을 다섯 단계로 나눠 1 초 1 줄:

    SINKPROF id=0 fr=30 busy_ms=802.8 (pace=792.1 diag=0.0 build=10.1 render=0.3 notify=0.2)

***`pace` 는 CPU 가 아니다*** — thread 페이싱에서 자는 시간이라 코어를 쓰지 않는다.
실제 CPU 는 나머지 넷이고, 개발기 4 영상에서 초당 ***10.6ms = 0.011 코어/영상*** 이다.

- `notify` 0.2ms/s — ***프레임마다 IpcSender 전송인데도 무시할 수준이다.*** 가장 굵어
  보이던 후보가 가장 얇았다.
- `render`(video_renderer) 0.3ms/s, `diag` 0.0.
- `build`(D3D11 업로드 포함) 10.1ms/s 로 이 중 대부분.

### 결론: 0.35 코어는 우리 코드 밖, GStreamer 디코드 경로에 있다

    스레드 총량            0.76 코어/영상
    우리 콜백 전체         0.01
    ------------------------------
    GStreamer 디코드 경로   0.75

그런데 ***같은 장비의 순수 디코드 천장은 0.36*** 이다. 즉 GStreamer 가 문맥 안에서
자기 천장의 2 배를 쓴다 — 그리고 그것은 ***이미 기준선에서 본 그 값***이다
(단일 프로세스 고정 0.60, 고정 없음 1.005, 54 프로세스 0.418).

***월 코드에서 깎을 것은 남아 있지 않다.*** 남은 것은 전부 "한 프로세스 안에서 GStreamer
파이프라인 N 개를 돌리면 개당 2 배가 든다"는 그 현상이고, 이 조사에서 그것을 없애는
방법으로 확인된 것은 ***프로세스를 나누는 것*** 뿐이다.


## ★★2026-08-27 (7) — "한 프로세스 2 배 페널티"의 정체: 물리 코어 고갈 + SMT★★

한 프로세스 안의 ***체인 수만*** 바꾸며(고정으로 NUMA 변수를 제거) 잰 곡선:

| 체인/프로세스 | 영상당 | 배율 | 총 CPU | 이 프로세스가 쓸 수 있는 물리 코어 |
|---|---|---|---|---|
| 6 | 0.389 | 1.11x | 2.34 | 20 중 2.3 |
| 12 | 0.451 | 1.27x | 5.42 | 20 중 5.4 |
| 20 | 0.472 | 1.33x | 9.45 | 20 중 9.5 |
| 27 | 0.479 | 1.36x | 12.93 | 20 중 12.9 |
| **40** | 0.600 | **1.70x** | **24.02** | ***20 초과*** |
| **54** | 0.735 | **2.09x** | **39.68** | ***논리 40 이 전부 참*** |

***무릎이 정확히 물리 코어 수에서 꺾인다.*** 고정된 프로세스는 한 소켓(물리 20 / 논리 40)만
쓴다. 총 수요가 물리 20 을 넘는 순간 SMT 짝이 둘 다 돌기 시작하고, 하이퍼스레드는 짝이
바쁘면 60~70% 속도로 떨어지므로 ***"코어를 더 썼다"로 측정된다.***

### 힙 경합도, GStreamer 전역 락도 아니었다

경합이 맞긴 하지만 상대가 다르다 — ***물리 코어와 SMT 파이프라인***이다. 이걸로 사다리
전체가 설명된다:

    1 프로세스 고정 없음  1.005   스레드가 먼 소켓으로 흩어짐 (원격 메모리)
    1 프로세스 고정       0.735   지역성은 얻고, 물리 20 코어에 갇힘 (SMT 포화)
    54 프로세스           0.418   두 소켓 40 물리 코어 + 각자 로컬 메모리

***서로 다른 두 손실이었다*** — 원격 메모리와 물리 코어 부족. 프로세스 분리는 둘 다
피했던 것이고, 하드 고정은 그중 하나만 고친다.

### 45 와 54 의 경계는 산술이다

경합 없는 in-process 비용이 약 0.45~0.48 코어/영상이다(6~27 체인 구간).

    물리 20 코어 / 0.45 = 약 44 영상   <- 한 소켓의 정직한 상한

***45 영상이 되고 54 영상이 안 되는 이유가 이것이다.*** GStreamer 에 병리적인 것은 없고
월 코드에도 깎을 것이 없다(콜백 전체가 영상당 0.011 코어). 장비의 한 소켓이 딱 그만큼이다.

### 남은 길

54 영상을 하려면 ***두 소켓의 물리 코어를 쓰면서 메모리는 로컬로*** 두어야 한다. 단일
프로세스로 그러려면 스레드별로 노드를 나눠 배치하고 힙도 그 노드에 잡아야 하는데,
GStreamer 가 스레드를 만들고 힙이 프로세스 공용이라 지금 구조에는 그 제어점이 없다.
대조군이 54 영상을 50~70% 로 도는 것도 두 소켓을 다 쓰기 때문일 것이다 —
0.45 x 54 = 24 코어는 한 소켓의 20 으로는 어떻게 해도 안 나온다.


# ★★★2026-08-27 — 54 영상 달성. 대조군보다 CPU 가 낮다★★★

`media_numa_pin_streaming_threads=true`(런처 `-NumaPin`) 실측, singlegpu 5760x2160,
uridecodebin3 + thread 페이싱:

| 54 영상 | 그룹 배치 | pts | 영상 fps | cadence | gap p50 | **머신 CPU** | 디코드/스레드 |
|---|---|---|---|---|---|---|---|
| 고정 없음 | 한 그룹 | 0.70 | 21.0 | 31.7 | 78.5 | **71.1%** | 0.99 SAT |
| 프로세스 고정 | 한 그룹 | 1.00 | 30.0 | **2.2** | — | 50.1% | 0.74 |
| **NumaPin** | **g0:13.8 + g1:14.1** | **1.00** | **30.0** | 42.3 | 54.3 | **35.4%** | **0.53** |
| **NumaPin + escape** | **g0:13.2 + g1:14.0** | **1.00** | **30.0** | 139.4 | **25.1** | **34.8%** | **0.50** |

***핀 54 개가 전부 걸렸고 두 그룹에 정확히 반씩 갈렸다.*** 예측(0.45~0.48 이 유지되면
25 코어)이 실측 0.50~0.53 / 27.2~27.9 코어로 맞았다.

### 대조군 대비

***대조군(비 Servo 월)이 같은 장비에서 54 영상을 50~70% 로 돈다. 우리는 34.8~35.4% 다.***
이 조사를 시작한 전제 — "대조군은 54 개, 우리는 42~45 개가 한계" — 가 뒤집혔다.
45 영상도 37.3(고정 없음) -> 34.4(프로세스 고정) -> ***22.0 코어(27.9%)*** 로 내려갔다.

### escape=external 의 평가가 다시 뒤집혔다

`-NumaPin` 위에 얹으면 지금까지 중 가장 좋다(육안 확인):

    draw_calls   56 -> 1        합성이 비디오 텍스처 54 장을 샘플링하지 않는다
    gap p50    54.3 -> 25.1ms   30fps 간격 33ms 보다 짧다 = 늦는 프레임이 사실상 없다
    16ms 초과  1249 -> 33 개

***앞서 escape=external 이 역효과였던 것은 프로세스를 한 노드에 가둬 렌더러가 굶던
상태였기 때문이다.*** 렌더러에 여유가 생기니 제 역할을 한다. 조합이 바뀌면 결론도
바뀐다 — 이 조사에서 세 번째로 확인한 교훈이다.
