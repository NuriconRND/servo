...
### 두 번째 이후 타일이 왜 빨라졌는가 (2026-09-04)

`?multi=3` 실측에서 소비자 추가 간격이 **정확히 7초**였다. 프로브가 의도적으로 넣은 간격은
0.8초뿐이므로 **약 6.2초가 `getUserMedia` 안에서** 쓰였다는 뜻이고, 그 구간의 로그는 완전히
조용했다.

원인은 허브 **앞단**이었다. `create_input_stream` 이 호출마다 이걸 했다:

- `GstDeviceMonitor` 를 새로 만들고 `monitor.devices()` 로 전 프로바이더를 프로브,
- `select_device_by_id` 가 후보마다 `device_path()` 를 부르는데, winks 장치는
  `device.path` 프로퍼티가 없어 **`ksvideosrc` 를 실제로 인스턴스화**해서 경로를 읽는다.

즉 허브는 "장치를 다시 여는 것"만 막았고, **"어느 장치인지 찾는" 비용은 타일마다 그대로**
치르고 있었다. 두 번째 타일부터는 답이 이미 열려 있는 허브인 게 확정인데도.

그래서 `deviceId → HubKey` 해석 결과를 기억한다(`capture_hub.rs` 의 `RESOLVED_IDS`).
같은 deviceId 가 다시 오면 열거를 건너뛰고 바로 소비자로 붙는다 —
**단, 그 키의 허브가 이미 열려 있고 healthy 할 때만.** 아니면 `None` 을 돌려주고 원래 경로로
떨어진다. 이 조건이 안전장치다: 낡은 항목이 죽은 장치를 되살릴 수는 없고, 살아있는 장치 앞의
일만 건너뛴다. 로그로 확인할 수 있다 —
...
> 첫 호출의 포맷을 받는다(§6 한계 참고).

### 프로브 페이지를 만들 때 — ★Servo 는 CSS Grid 가 기본으로 꺼져 있다★

`components/config/prefs.rs` 의 `layout_grid_enabled` 기본값이 `false` 다. 그래서
`display: grid` 는 조용히 `display: block` 으로 폴백하고, 셀마다 전체 폭 블록이 되어
**첫 타일이 벽 전체를 먹고 나머지는 화면 밖으로 밀린다.** 에러도 경고도 없다.

`-Pref layout_grid_enabled=true` 로 켤 수는 있지만, **프로브가 자기 레이아웃을 위해 pref 를
요구하면 안 된다** — 그 pref 를 빠뜨린 실행이 "동시 표출이 안 된다"처럼 보이기 때문이다.
동시 프로브는 JS 로 계산한 퍼센트 절대 위치를 쓴다(박스 모델 외에는 아무것도 안 쓴다).
