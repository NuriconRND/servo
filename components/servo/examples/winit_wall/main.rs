...
fn parse_args() -> Result<Config, Box<dyn Error>> {
    ...
    // `WallLayoutError` (shared `servo::wall_layout` crate) does not implement
    // `std::error::Error`, so it can't cross a bare `?` into `Box<dyn Error>`; stringify it.
    let layout =
        WallLayout::from_path(Path::new(&layout_path)).map_err(|error| error.to_string())?;
    if !all_tiles {
        layout
