...
use servo_canvas_traits::webgl::{
    ...
    WebGLContextNotification, WebGLCreateContextResult, WebGLError, WebGLFramebufferBindingRequest,
    WebGLMsg, WebGLMsgSender, WebGLProgramId, WebGLResult, WebGLSLVersion, WebGLVersion,
    YAxisTreatment, webgl_channel,
};
...
pub(crate) fn report_live_contexts() {
    LIVE_CONTEXTS.with(|live| {
        ...
        // 살아 있는 컨텍스트들이 붙들고 있는 GPU 바이트의 하한. ★컨텍스트 하나는 Rust 로는
        // 수백 바이트지만 GPU 로는 그리기 버퍼 여러 장이다★ — 그 값을 같이 내지 않으면
        // "live=4" 가 얼마나 큰 일인지 로그만 보고는 알 수 없다. 팬아웃 벽에서는 타일마다
        // 서피스가 따로 있으므로 실제 점유는 이 값보다 크다(여기서는 타일 수를 모르므로
        // 곱하지 않는다 — 하한임을 이름으로 밝힌다).
        let mut buffer_bytes: u64 = 0;
        // ★같은 문서에 쌓이는 것과, 문서 자체가 안 헐리는 것은 다른 고장이다★ — 앞은
        // 페이지가 옛 캔버스를 들고 있는 것이고, 뒤는 옛 문서(파이프라인)가 살아 있는
        // 것이다. 뒤라면 CPU 메모리도 같이 늘고, 그 안의 것은 무엇을 해도 수집되지 않는다.
        // `is_connected` 는 "뿌리가 Document 인가" 만 보므로 둘을 구분하지 못한다.
        ...
        for context in live.iter() {
            ...
            let size = context.size.get().cast::<u64>();
            buffer_bytes += size.width * size.height * 4;
            if let HTMLCanvasElementOrOffscreenCanvas::HTMLCanvasElement(canvas) = &context.canvas {
...
pub(crate) struct WebGLRenderingContext {
    ...
    /// 이 컨텍스트의 GPU 자원을 회수해 둔 상태인가.
    ///
    /// 화면에서 오래 사라진 컨텍스트는 백엔드를 놓는다(벽에서는 타일마다 한 벌씩이라
    /// 네 벌이다). 놓으면 GL 상태도 함께 사라지므로, 표준대로 페이지에
    /// `webglcontextlost` 를 알리고 그동안의 명령은 버린다 — 없는 백엔드로 보내면
    /// 그대로 잃는 명령이고, 페이지는 `webglcontextrestored` 에서 다시 그린다.
...
impl WebGLRenderingContext {
    ...
    pub(crate) fn new(
        ...
        context.listen_for_context_notifications(window);
        Some(context)
    }

    /// 손실/복구 통지를 받아 스크립트 스레드에서 처리하도록 건다.
    ///
    /// 생성 응답은 일회성이라 그 뒤로 WebGL 스레드가 말을 걸 길이 없었다. 자원을 회수하는
    /// 쪽은 "보이는가" 를 아는 페인트/WebGL 스레드이고, 이벤트를 쏘는 쪽은 여기이므로 그
    /// 사이를 잇는 채널이 필요하다.
    fn listen_for_context_notifications(&self, window: &Window) {
        let Ok((sender, receiver)) = ipc::channel::<WebGLContextNotification>() else {
            warn!("WebGL: could not open a context-notification channel");
            return;
        };
        self.droppable.webgl_sender.set_context_notifier(sender);
    ...
    /// 손실/복구를 페이지에 알린다. 표준이 정한 이름 그대로다.
    fn handle_context_notification(
        &self,
        cx: &mut js::context::JSContext,
        message: WebGLContextNotification,
    ) {
        let (lost, name) = match message {
            WebGLContextNotification::Lost(_) => (true, Atom::from("webglcontextlost")),
            WebGLContextNotification::Restored(_) => (false, Atom::from("webglcontextrestored")),
        };
        if self.context_lost.get() == lost {
            return;
        }
        self.context_lost.set(lost);
        let HTMLCanvasElementOrOffscreenCanvas::HTMLCanvasElement(canvas) = &self.canvas else {
            return;
        };
        ...
        let event = WebGLContextEvent::new(
            ...
            name,
            EventBubbles::DoesNotBubble,
            // 손실은 취소 가능하다(페이지가 preventDefault 로 복구를 요청한다). 복구는 아니다.
            if lost {
                EventCancelable::Cancelable
            } else {
    ...
    pub(crate) fn send_with_fallibility(&self, command: WebGLCommand, fallibility: Operation) {
        // 자원을 놓아 둔 동안의 명령은 보내지 않는다 — 받을 백엔드가 없다. 표준도 잃은
        // 컨텍스트에서는 호출이 아무 일도 하지 않게 정한다.
