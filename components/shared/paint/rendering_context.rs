...
/// One physical display (a DXGI output) and the GPU adapter that drives it.
///
/// `adapter_index` is the DXGI `EnumAdapters1` order index — the exact value
/// [`create_adapter_for_requested_gpu`] / `create_dxgi_adapter_by_index` consume as a
/// `requested_gpu_index` — so a tile shown on this display can bind to the GPU that drives
/// it by passing `adapter_index` straight through. `luid` is the matching `AdapterLuid`
/// (see [`dxgi_luid_for_gpu_index`]); `left/top/width/height` are the output's desktop
/// virtual coordinates in physical pixels.
#[derive(Clone, Debug)]
...
mod display_topology_tests {
    ...
    #[test]
    fn row_major_2x1() {
        // Enumeration order arbitrary/reversed; spatial order must be left→right.
        let displays = vec![
    ...
    fn mixed_resolution_row_bands_together() {
        // A 4K and a 1080p side by side share a row despite different heights.
        let displays = vec![
