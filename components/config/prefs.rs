...
pub struct Preferences {
    ...
    /// 않은 이유다(설계 문서 `## 실기 결과`).
    ///
    /// ★대가를 분명히 한다.★ 이음매 오차가 사라지는 것이 아니라 **상수가 된다**:
    /// `속도 × 출력 간 위상차`. 500px/s 에서 5.6px, 1x 프로브(2,304px/s)에서 26px.
    /// 그 고정 어긋남은 회귀가 아니라 이 거래의 내용이고, 판정 기준은 "이음매가 붙었나" 가
    /// 아니라 "타일 안에서 등속으로 매끄럽나" 다. 베젤 없는 완전 동기는 genlock 없이
    /// 성립하지 않는다.
    ///
    /// 값이 네 타일에 공통이므로 이음매 정렬에는 영향이 없다 -- 콘텐츠 전체가 이르냐
    /// 늦냐만 바꾼다. 실측으로 쓸어 정한다.
    pub gfx_sample_lead_periods: i64,
    ...
    /// 한 렌더 프레임에서 **새 비디오의 D3D11 링을 처음 잡는 일**을 몇 개까지 할지.
    /// `0` 이면 무제한(예전 동작).
    ///
    /// 링의 최초 소비는 슬롯 4개 × 면 3개 = D3D11 `Map` 12회이고 실측 중앙값 8.8ms,
    /// 최대 87ms 다. 이 일이 렌더러 스레드에서 **프레임 안에** 있는데, 전환 때 영상이
    /// 무더기로 뜨면 한 프레임에 21건이 몰려 그 프레임이 190~380ms 가 된다 -- 60Hz 장비에서
    /// 프레임 하나가 190ms 라는 것 자체가 고장이고, 표출 클럭이 같은 스레드에 있으므로
    /// 그동안 타일 넷이 통째로 멈춘다. 사용자가 보는 애니메이션 정지가 이것이다.
    ///
    /// 프레임당 하나로 묶으면 그 프레임은 ~9ms 늘 뿐이고 나머지는 다음 프레임으로 밀린다
    /// -- 즉 부푼 프레임이 **애초에 만들어지지 않는다**. 아직 차례가 오지 않은 영상은 그
    ...
    /// 3.1MB 를 초당 30 회 x 영상 수만큼 소켓 간 링크로 나른다. 반대로 프로세스를
    /// 한 노드에 하드 고정하면 지역성은 얻지만 물리 코어가 절반(20)으로 줄어
    /// SMT 가 포화한다 — 실측(54xFHD30, 코어/영상): 고정 없음 1.005, 프로세스
    /// 고정 0.735, 54 개 별도 프로세스 0.418.
    ///
    /// 스레드를 노드에 붙박이로 두면 둘 다 얻는다: 그 스레드가 할당한 버퍼가 그
    /// 노드에 잡히고 같은 스레드가 읽으며(Windows 힙의 스레드별 캐시가 돕는다),
