param(
    ...
    [string] $TileSize = "",
    [string] $LogPrefix = "video_wall_d3d11_debug",
...
# Native Compositor (DirectComposition). WR draws picture-cache tiles directly into DComp surfaces and DWM
...
$servoExe  = Join-Path $servoRoot "target\debug\servoshell.exe"
...
if (!(Test-Path $servoExe)) { throw "servoshell.exe not found (build first: .\mach build): $servoExe" }
...
$argumentList = @("--window-size", $WindowSize) + $prefArgs + @($url)
